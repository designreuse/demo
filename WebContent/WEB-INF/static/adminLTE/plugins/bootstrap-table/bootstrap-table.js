...
! function ($) {
	...
	BootstrapTable.DEFAULTS = {
		...
		showFooter: false,
		showColumns: false,
		showExportType: false, //TODO 试用指令的方式显示导出按钮方式
	...
	BootstrapTable.prototype.load = function (data) {               //TODO 修改列表返回结果
		...
		if (this.options.sidePagination === 'server') {
			if(data.content==undefined){
				...
				this.options.totalRows = data.totalElements;
				fixedScroll = data.fixedScroll;
				data = data.content;
			}


		} else if (!$.isArray(data.content)) { // support fixedScroll
			fixedScroll = data.fixedScroll;
			data = data.content;
		}
	...
	BootstrapTable.prototype.exportLoad = function (data) { //TODO 修改导出结果
		var fixedScroll = false;
	...
	//TODO 修改源码新增此方法，保证导出全部时，数据请求会来后再导出
	BootstrapTable.prototype.togglePaginationAll = function (orgPageSize,funcBack) {
		...
		request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
			...
			success: function (res) {
				...
				that.options.pageSize = orgPageSize;
				funcBack();
			},
			error: function (res) {
				that.trigger('load-error', res.status);
	...
	BootstrapTable.prototype.refresh = function (params) {
		$('.ui-btn-enable').each(function(){                            //TODO 修改table刷新后操作按钮也同时刷新
